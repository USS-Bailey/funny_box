...
void TIM3_Breathing(uint32_t lamp_count)
{
	TIM_OCInitTypeDef  TIM_OCInitStructure;

  TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM1;	              //配置为PWM模式1
  TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;  	//选择使能输出比较状态
  TIM_OCInitStructure.TIM_Pulse = lamp_count;	      							//设置待装入通道3捕获比较寄存器的脉冲值,取值范围0x0000~0xFFFF 
  TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High;       //设置通道3输出比较极性高电平
  TIM_OC3Init(TIMx, &TIM_OCInitStructure);	                      //使能TIM2通道3
  TIM_OC3PreloadConfig(TIMx, TIM_OCPreload_Enable);               //使能TIM2在CCR3上的预装载寄存器
